...
mail_data['money_keyword'] = mail_data['text'].apply(lambda x: 0 if any(check_keywords(x)) else 1)
...
mail_data['contains_money_keywords'] = mail_data['text'].apply(lambda x: 0 if contains_money_keywords(x) else 1)

# Label spam mail as 0; ham mail as 1
# mail_data.loc[mail_data['target'] == 'spam', 'Category',] = 0
# mail_data.loc[mail_data['target'] == 'ham', 'Category',] = 1

# Separating the data as texts and label
# X = mail_data['Message'].apply(lambda x: word_tokenize(x))
mail_data['text'] = mail_data['text'].apply(process_email)
mail_data['EmailLength'] = mail_data['emailAddress'].apply(lambda x: len(x))

X = mail_data[['text', 'contains_money_keywords', 'money_keyword','EmailLength']].values.tolist()

Y = mail_data['target']
email_counts = mail_data['target'].value_counts()
